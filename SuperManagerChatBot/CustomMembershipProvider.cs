...
namespace SuperManagerChatBot
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            User user = new User();
            dt = (new DBAccess()).GetRcdSetByQry("SELECT * FROM TBL_USER WHERE EMAILID='" + username + "';");
            if (dt.Rows.Count > 0)
            {
                Role role = new Role();
                role.RoleId = 1;
                ...
                ICollection<Role> roles = new[] { role };
                user.Roles = "User";
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                CustomPrincipal customPrincipal = new CustomPrincipal(dt.Rows[0]["EmailID"].ToString());
                return true;
            }
            else
            {
